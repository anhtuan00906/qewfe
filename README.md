...
const config = {
  ...
  async function connectUptime(url, name) {
    try {
      const res = (await axios.get(`https://uptime.miraiprofile2005.repl.co/?add=${url}`)).data;
...
function onBot({ models }) {
    ...
    if (process.env.KEY && global.config.encryptSt) {
      ...
      writeFileSync(appStateFile, loginState)
        ...
        const listener = require('./includes/listen')(listenerData); //dong này là sơ xuất nhỏ của sơn vì file người ta nền chỉ sửa xíu cho nó chạy mượt mà tí thôi
